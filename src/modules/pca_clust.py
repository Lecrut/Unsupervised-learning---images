...
#%% PCA reductions:
def pca_reduction(latents, n_components=50):
    print(f"Redukcja PCA do {n_components} wymiarów...")
    pca = PCA(n_components=n_components)
    reduced_data = pca.fit_transform(latents)
...
def find_optimal_clusters_gmm(latents, max_k=20):
    ...
    # Spłaszczanie jeśli trzeba (dla pewności)
    if len(latents.shape) > 2:
        latents = latents.reshape(latents.shape[0], -1)

    # 2. Pipeline: Normalize -> PCA -> UMAP
    print("Szukanie optymalnego K (Pipeline: Norm -> PCA -> UMAP -> GMM)...")
    ...
    X_reduced = reducer.fit_transform(X_pca) # UMAP bierze teraz wynik PCA
    
    # Próbkowanie dla szybkości (jeśli dużo danych)
    ...
    for k in tqdm(K_range, desc="Analiza GMM"):
    ...
    ax1.set_title('BIC Score (Szukaj "łokcia")')
    ax1.grid(True, alpha=0.3)
    
    ax2.plot(K_range, silhouettes, 'ro-')
    ax2.set_xlabel('Liczba klastrów (k)')
    ax2.set_ylabel('Silhouette Score')
    ax2.set_title('Silhouette Score (Im wyżej tym lepiej)')
...
#%% Predict clusters
def predict_clusters(model, dataloader, pca_model, umap_model, gmm_model, device):
    model.eval()
    all_labels = []
    
    print("Przewidywanie klastrów dla zbioru danych...")
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Inference"):
            # Obsługa batcha (czy jest listą [img, label] czy samym tensorem)
            imgs = batch[0].to(device) if isinstance(batch, (list, tuple)) else batch.to(device)
            
            # 1. Encoder: [Batch, 64, 8, 8]
            latents, _ = model.encoder(imgs)
            
            # 2. Flatten: [Batch, 2048]
            latents = latents.view(latents.size(0), -1)
            
            # Konwersja do Numpy (wymagana przez sklearn/umap)
            latents_np = latents.cpu().numpy()
            ...
            # 4. Transformacja PCA
            latents_pca = pca_model.transform(latents_norm)
            
            # 5. Transformacja UMAP
            latents_umap = umap_model.transform(latents_pca)
            
            # 6. Predykcja GMM
            batch_labels = gmm_model.predict(latents_umap)
